...
export default class LineComp extends React.Component {
  render() {
    ...
    return <Line data={this.props.data} options={customBarOptionToolTip} />;
  }
}
  ...
  if (tooltip.body) {
    ...
    for (let i = 0; i < 1; i++) {
      for (let key in tooltipData.value[tooltipData.xLabel][0]) {
        if (
          ...
          tooltipData.value[tooltipData.xLabel][0][key]
        ) {
          colHeaderName.push(
    ...
    for (let i = 0; i < dataLength; i++) {
      ...
      for (let j = 0; j < colHeaderName.length; j++) {
        ...
        if (colHeaderName[j].indexOf("date") > 1) {
          let dateTime =
            tooltipData.value[tooltipData.xLabel][i][
              colHeaderName[j].toLowerCase().replace(" ", "_")
            ];
          dateTime = moment.utc(dateTime).format("DD-MMM-YYYY");

          tabCell.innerHTML = dateTime;
        } else {
          tabCell.innerHTML =
            tooltipData.value[tooltipData.xLabel][i][
              colHeaderName[j].toLowerCase().replace(" ", "_")
