...
import "../App.css";
// const onlyUnqiueValue = (value, index, self) => {
//   return self.indexOf(value) === index;
// };

// var labels = [];
// var records = JSON.parse(JSON.stringify(jsonData))[0].records
//   ? JSON.parse(JSON.stringify(jsonData))[0].records
//   : [];
// for (let i = 0; i < records.length; i++) {
//   let obj = records[i];
//   const date = moment(obj.end_date).add(i, "days").format("YYYY-MM-DD");
//   labels.push(date.toString());
// }

// labels = labels.filter(onlyUnqiueValue);
...
const label = groupBy("end_date", originalData, "groupKeyVal");
const sourceData = [];
Object.keys(label)
  .reverse()
  .forEach(function (key) {
    sourceData.push(label[key].length);
  });
    ...
    for (let i = 0; i < 1; i++) {
      for (let key in tooltipData.value[tooltipData.xLabel][0]) {
        if (
          ...
          colHeaderName.push(key.toUpperCase().replace("_", ""));
        }
      }
    }
...
const customBarOptionToolTip = {
  ...
  allDateVal: Object.keys(label).map((e) => {
    return new Date(e);
  }),
};
...
export default class LineChart extends React.Component {
  ...
  onDateChangeEvenetListener = (startDate, endDate, data) => {
    ...
    var labelData = groupBy("end_date", fixData, "groupKeyVal");
    var labels = Object.keys(labelData).sort();
    var gData = []; //groupBy("end_date", fixData);

    Object.keys(labelData)
      .reverse()
      .forEach(function (key) {
        gData.push(labelData[key].length);
      });
